...
export interface ResultadosGridRef {
  handleSaveAll: () => Promise<void>;
}

const ResultadosGrid = forwardRef<ResultadosGridRef, ResultadosGridProps>(
  (
    {
      atletas,
      config,
    ...
    const esSuperJuez = useAuthStore(
      (s) => s.juezSession?.esSuperJuez ?? false,
    );
    ...
    const colDefs = useMemo<ColDef<GridRow>[]>(
      () => buildColDefs(config, esSuperJuez, alturas, pruebaName),
      [config, esSuperJuez, alturas, pruebaName],
    );
...
ResultadosGrid.displayName = "ResultadosGrid";
